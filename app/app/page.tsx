...
interface DashboardStats {
  ...
  genreSuccessTrend?: { year: number; successCount: number; avgRating: number }[];
  // Stats
  correlationRatingsVotes: { rating: number; votes: number; title: string }[];
  chiSquareTest: { genre: string; awardWinners: number; nonWinners: number; chiSquare: number; pValue: number; significant: boolean }[];
...
export default function Dashboard() {
  ...
  const fetchDashboard = async (genre = selectedGenre, decade = selectedDecade) => {
    const query = `?genre=${encodeURIComponent(genre)}&decade=${encodeURIComponent(decade)}`;
  ...
  const handleBestGenreDecadeChange = async (decade: string) => {
    ...
    try {
      const res = await fetch(`/api?bestGenreDecade=${encodeURIComponent(decade)}`);
  ...
  const handleActorSearch = async (evt: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`/api?actorName=${encodeURIComponent(actorName)}`);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-blue-900 to-gray-900 p-8">
        ...
        <div className="mb-12">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-8">
            ...
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-6">
              <div className="flex items-center mb-6">
                ...
                <div>
                  <h3 className="text-xl font-bold text-gray-800 dark:text-white">Popular Actors</h3>
                  <p className="text-xs text-gray-500 dark:text-gray-400">Roll-up by movie count</p>
                </div>
              </div>
              {stats.popularActors.length > 0 ? (
                <div className="space-y-3">
                  {stats.popularActors.map((actor, index) => (
                    <div key={index} className="flex justify-between items-center p-3 bg-gradient-to-r from-gray-50 to-pink-50 dark:from-gray-700/50 dark:to-pink-900/20 rounded-xl">
                      <div>
                      ...
                      <div className="text-xl font-bold text-pink-600">⭐ {actor.avgRating.toFixed(1)}</div>
                    </div>
                  ))}
                </div>
              ) : (
            ...
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-6">
              <div className="flex items-center mb-6">
                ...
                <div>
                  <h3 className="text-xl font-bold text-gray-800 dark:text-white">Popular Genres</h3>
                  <p className="text-xs text-gray-500 dark:text-gray-400">Roll-up by volume</p>
                </div>
              </div>
              {stats.genreDistribution.length > 0 ? (
                <ResponsiveContainer width="100%" height={350}>
                  <BarChart data={stats.genreDistribution}>
                    ...
                    <XAxis dataKey="name" tick={{ fill: '#6b7280', fontSize: 10 }} angle={-20} textAnchor="end" height={80} />
                    <YAxis tick={{ fill: '#6b7280' }} />
                    <Tooltip contentStyle={{ backgroundColor: 'rgba(100, 100, 100, 0.95)', border: 'none', borderRadius: '12px' }} />
            ...
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-6">
              <div className="flex items-center mb-6">
                ...
                <div>
                  <h3 className="text-xl font-bold text-gray-800 dark:text-white">Popular Movies of Actor</h3>
                  <p className="text-xs text-gray-500 dark:text-gray-400">Roll-up, Slice by actor</p>
                </div>
              </div>
              ...
              {stats.actorProfile ? (
                <div className="space-y-3">
                  <div className="p-3 bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-gray-700/50 dark:to-blue-900/20 rounded-xl mb-3">
                    ...
                    <div className="text-xs text-gray-600 dark:text-gray-400">
                      {stats.actorProfile.totalMovies} movies • ⭐ {stats.actorProfile.avgRating.toFixed(1)}
                    </div>
                  </div>
                    ...
                    <table className="w-full text-xs">
                      <thead className="sticky top-0 bg-gray-100 dark:bg-gray-700">
                        <tr className="border-b border-gray-200 dark:border-gray-600">
                          <th className="text-left p-2 font-semibold text-gray-700 dark:text-gray-300">Movie</th>
                          <th className="text-center p-2 font-semibold text-gray-700 dark:text-gray-300">Year</th>
                          <th className="text-center p-2 font-semibold text-gray-700 dark:text-gray-300">Rating</th>
                        </tr>
                      </thead>
                      <tbody>
                        {stats.actorProfile.recentMovies.slice(0, 10).map((movie, index) => (
                          <tr key={index} className="border-b border-gray-100 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/50">
                            <td className="p-2 text-gray-800 dark:text-white font-medium">{movie.title}</td>
                            <td className="text-center p-2 text-gray-600 dark:text-gray-400">{movie.year}</td>
                            <td className="text-center p-2 font-semibold text-blue-600 dark:text-blue-400">⭐ {movie.rating.toFixed(1)}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                ...
                <p className="text-gray-400 text-center py-8 text-sm">Search for an actor above</p>
              )}
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-6">
            <div className="flex items-center mb-6">
              ...
              <div>
                <h3 className="text-xl font-bold text-gray-800 dark:text-white">Count Oscars by Canonical Category</h3>
                <p className="text-xs text-gray-500 dark:text-gray-400">Drill Down by award category</p>
              </div>
            </div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b-2 border-gray-200 dark:border-gray-700">
                      <th className="text-left p-3 font-semibold text-gray-700 dark:text-gray-300">Genre</th>
                      <th className="text-right p-3 font-semibold text-gray-700 dark:text-gray-300">Winners</th>
                      <th className="text-right p-3 font-semibold text-gray-700 dark:text-gray-300">Non-Winners</th>
                      <th className="text-right p-3 font-semibold text-gray-700 dark:text-gray-300">χ²</th>
                      <th className="text-right p-3 font-semibold text-gray-700 dark:text-gray-300">Result</th>
                    </tr>
                  </thead>
                  <tbody>
                    {stats.chiSquareTest.map((row, index) => (
                      <tr key={index} className="border-b border-gray-100 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/50">
                        <td className="p-3 font-semibold text-gray-800 dark:text-white">{row.genre}</td>
                        <td className="text-right p-3 text-gray-600 dark:text-gray-300">{row.awardWinners}</td>
                        <td className="text-right p-3 text-gray-600 dark:text-gray-300">{row.nonWinners}</td>
                        <td className="text-right p-3 font-mono text-gray-800 dark:text-white">{row.chiSquare.toFixed(2)}</td>
                        <td className="text-right p-3">
                          <span className={`px-2 py-1 rounded-full text-xs font-semibold ${row.significant ? "bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400" : "bg-gray-100 text-gray-700 dark:bg-gray-700 dark:text-gray-300"}`}>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-6">
              <div className="flex items-center mb-6">
                ...
                <div>
                  <h3 className="text-xl font-bold text-gray-800 dark:text-white">Best Film Genres</h3>
                  <p className="text-xs text-gray-500 dark:text-gray-400">Slice - Success by decade</p>
                </div>
              </div>
              
              {/* Decade Selector */}
              <div className="mb-4">
                <label className="block text-xs uppercase tracking-wide text-gray-500 dark:text-gray-400 mb-2">Select Decade</label>
                <select
            ...
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-6">
              <div className="flex items-center mb-6">
                ...
                <div>
                  <h3 className="text-xl font-bold text-gray-800 dark:text-white">Professions</h3>
                  <p className="text-xs text-gray-500 dark:text-gray-400">Roll-up pie chart</p>
                </div>
              </div>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-6">
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center">
                  ...
                  <div>
                    <h3 className="text-xl font-bold text-gray-800 dark:text-white">Successful Movies</h3>
                    <p className="text-xs text-gray-500 dark:text-gray-400">Dice - by genre & decade</p>
                  </div>
                </div>
              </div>

              <form onSubmit={handleTrendSubmit} className="flex flex-wrap items-end gap-2 mb-4">
                <div className="flex-1 min-w-[100px]">
                  <label className="block text-xs uppercase tracking-wide text-gray-500 dark:text-gray-400 mb-1">Genre</label>
                  <input
                    value={formGenre}
                ...
                <div className="flex-1 min-w-[100px]">
                  <label className="block text-xs uppercase tracking-wide text-gray-500 dark:text-gray-400 mb-1">Decade</label>
                  <select
                    value={formDecade}
              ...
              {trendLoading ? (
              ...
              ) : stats.genreSuccessTrend && stats.genreSuccessTrend.length > 0 ? (
                <ResponsiveContainer width="100%" height={250}>
                  <LineChart data={stats.genreSuccessTrend}>
                    ...
                    <Tooltip contentStyle={{ backgroundColor: 'rgba(100, 100, 100, 0.95)', border: 'none', borderRadius: '12px', fontSize: '12px' }} />
                    <Legend wrapperStyle={{ fontSize: '11px' }} />
                    <Line yAxisId="left" type="monotone" dataKey="successCount" stroke="#10B981" strokeWidth={3} dot={{ r: 4 }} name="Success Count" activeDot={{ r: 6 }} />
                    <Line yAxisId="right" type="monotone" dataKey="avgRating" stroke="#3B82F6" strokeWidth={3} dot={{ r: 4 }} name="Avg Rating" activeDot={{ r: 6 }} />
                  </LineChart>
                </ResponsiveContainer>
              ) : (
            ...
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-6">
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center">
                  ...
                  <div>
                    <h3 className="text-xl font-bold text-gray-800 dark:text-white">Correlation Test</h3>
                    <p className="text-xs text-gray-500 dark:text-gray-400">Roll-up - Ratings vs Votes</p>
                  </div>
                </div>
                <div className="text-right">
                  <p className="text-sm text-gray-500 dark:text-gray-400">Pearson r</p>
                  <p className="text-2xl font-bold text-blue-600">{stats.pearsonCorrelation?.toFixed(4) || 'N/A'}</p>
                </div>
              </div>
              {stats.correlationRatingsVotes.length > 0 ? (
                <ResponsiveContainer width="100%" height={300}>
                  <ScatterChart>
                    ...
                    <Tooltip cursor={{ strokeDasharray: '3 3' }} contentStyle={{ backgroundColor: 'rgba(255, 255, 255, 0.95)', border: 'none', borderRadius: '12px' }} />
      ...
      <div className="text-center text-gray-400 text-sm py-6 border-t border-gray-700">
        <p>📊 IMDb Analytics Dashboard • Built with Next.js & Recharts • MySQL Database</p>
        <p className="text-xs mt-2 text-gray-500">Roan Cedric Campo • STADVDB Project</p>
      </div>
    </div>
  );
}
